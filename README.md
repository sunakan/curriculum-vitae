...
- toB案件
- 10月末開始で1度11月末までに第1リリースする必要がありました。
...
-  メインで触るRailsがserviceとして定義しているAPIクライアント系のほとんどが返り値にHashie::Mashを使っており、ものすごく追いにくい状況にありました。（どういう形のjsonかがcallしないとわからないことに加えて、更にデータ構造が動的でした。）
- ちょくちょくエラーが握りつぶされてnilが返っていたり、空配列が返っていたりしていいました。
- 関わるサブプロダクトの方はREADME等のドキュメントがない状況でした。

#### どのように解決したか

- 状況を打破するべく、今回用意するAPIにはクライアント専用のエラーモデル、API用のクラスを別途作成した。例外処理も逐一投げるような処理にしました。
- 今回の経験を通して、初めて触る人がすぐに開発に着手できるよう、READMEを記述しました。
